...
public class Club {
    ...
    public List clubPlayerList(Club c,List<Player>playerList){
        ...
        this.playerList = clubPlayers;
        return clubPlayers;
    }

    public List getUpdatedPlayers(){
        return this.playerList;
    }

    public List getclubPlayerList(Club c,List<Player>playerList){
        List<Player>clubPlayers = new ArrayList();
        for(Player p : playerList){
            if(p.getClub().equals(c.getclubName())){
                clubPlayers.add(new Player(p.getName(),p.getCountry(),p.getAge(),p.getHeight(),p.getPosition(),p.getNumber(),p.getSalary()));
            }
        }
        return clubPlayers;
    }
    ...
    public double getMaximumSalary(){
        maximumSalary = 0.0;//playerList.get(0).getSalary();
        for(Player p : playerList){
